...
### Using Random Forest we can determine the most important features {data-commentary-width=400}
...
With the data a random forest was made which gives us the best-quality ranking of feature importance. We see that timbre components c01, c02 and c08, loudness, speechiness and danceability are the most important features. The accuracy, Cohen's kappa and the J-measure are still not very high as can be seen from the table.
...
The "elbow" in the previous plot indicates that the most important features are c01, c02 and c08, loudness, speechiness and danceability. As can be seen from the heat map, the predictions for John Mayer and Stevie Ray Vaughan are mostly correct. We see that there is more similarity for John Mayer and Stevie Ray Vaughan to Jimi Hendrix than to eachother. The predictions for Jimi Hendrix are not very accurate which indicates that some John Mayer and Stevie Ray Vaughan songs are similar.
...
From this plot we see that there is a better distinction between the three artist but that there is definitely some overlap.
